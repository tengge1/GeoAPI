namespace GeoAPI.Geometries.Prepared
{
    ...
    /// <remarks>
    /// <para>
    /// 给定的实现可以为仅一些指定的方法提供优化的实现，并且将剩余的方法委托给原始的
    /// <see cref =“IGeometry”/>操作。
    /// </para>
    /// <para>
    /// 实施也可能仅仅优化某些情况，并且委托他人。 请参阅实现类的文档，了解哪些方法和优化
    /// 方式。
    /// </para>
    /// <para>
    /// 子类旨在是线程安全的，以允许在多线程上下文中使用（c）IPreparedGeometry </ c>（允许
    /// 从准备状态中提取最大利益）。
    /// </para>
    /// </remarks>
    ...
    public interface IPreparedGeometry
    {
        ...
        /// 由于<tt>交集</ tt>是一个相当慢的操作，因此使用<tt> containsProperly </ tt>可以
        /// 更有效地过滤完全位于该区域内的测试几何。 在这些情况下，交叉点先验知道是简单的原
        /// 始测试几何。
        /// </para>
        /// </remarks>
